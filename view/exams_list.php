			...
			<div class="pd-ltr-20 xs-pd-20-10">
				<div class="min-height-200px">
					...
					<div class="card-box mb-30">
						<div class="pd-20">
							<h4 class="text-blue h4">Export the Exam list</h4>
							<?php
if (isset($_POST['submit'])) {
    $currentDate = $_POST['currentDate'];
    $nextExamDate = date('Y-m-d', strtotime($currentDate . ' +15 days'));
} else {
    $nextExamDate = date('Y-m-d', strtotime('+15 days'));
}
?>

<form method="POST">
    <label for="currentDate">Last exam date:</label>
    <input type="date" id="currentDate" name="currentDate" value="<?php echo $nextExamDate; ?>" required>
    <input type="submit" name="submit" value="Next exam date">
</form>

<?php
if (isset($_POST['submit'])) {
    echo "<p>Exam Date: $nextExamDate</p>";
}
?>
						</div>
						<div class="pb-20">
							...
							>
								...
								<thead>
									<tr>
										<th class="table-plus datatable-nosort">Student_ID</th>
		...
		<form id="add_result_form" method="post" action="index.php?action=add_result">
		<?php foreach ($items as $item): ?>
			<tr>
				<td class="table-plus"><?php echo htmlspecialchars($item['Student_ID']); ?></td>
				<td><?php echo htmlspecialchars($item['First_name']); ?></td>
				<td><?php echo htmlspecialchars($item['Last_name']); ?></td>
				<td><?php echo htmlspecialchars($item['Exam_stage']); ?></td>
