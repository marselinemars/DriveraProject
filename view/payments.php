					...
					<div class="page-header">
						<div class="row">
							...
							<div class="col-md-6 col-sm-12 text-right">
								<div class="dropdown">
									<a class="btn btn-primary dropdown-toggle" href="index.php?action=add_payment" role="button">
									  Add Payment For your Students
									</a>
								  </div>
...
<?php 
global $i;
$rowStyle = ''; // Initialize the row style variable

;

for ($i = 0; $i < count($pay); $i++) {
    $idpayment = $pay[$i]['PaymentID'];

    // Reset the row style for each iteration
    $rowStyle = '';

    if ($pay[$i]['RowStyle'] === 'red') {
        $rowStyle = 'red-row';
    } elseif ($pay[$i]['RowStyle'] === 'pink') {
        $rowStyle = 'pink-row';
    }

    // Add the row style class to the table row
    echo '<tr class="' . $rowStyle . '">';

    echo '<td class="table-plus">' . htmlspecialchars($pay[$i]['fname']) . '</td>';
    echo '<td>' . htmlspecialchars($pay[$i]['lname']) . '</td>';
    echo '<td>' . htmlspecialchars($pay[$i]['PaymentAmount']) . '</td>';
    echo '<td>' . htmlspecialchars($pay[$i]['PaymentDate']) . '</td>';
    echo '<td>' . htmlspecialchars($pay[$i]['RemainingAmount']) . '</td>';
    echo '<td>' . htmlspecialchars($pay[$i]['NextPaymentDate']) . '</td>';
    ...
    echo '<a class="dropdown-item" href="index.php?action=edit_payment&idpayment=' . $idpayment . '"><i class="dw dw-edit2"></i> Edit payment</a>';
    echo '<a class="dropdown-item" href="index.php?action=delete_payment&idpayment=' . $idpayment . '"><i class="dw dw-delete-3"></i> Delete</a>';
    echo '</div>';
    echo '</div>';
    echo '</td>';
