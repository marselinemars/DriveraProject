...
function insertSuccStudents() {
    ...
    $deletePassedQuery = "DELETE FROM succ_students WHERE studentID IN (
        SELECT Student_ID FROM exam WHERE Result = 'failed'
    )";
    ...
    $selectQuery = "SELECT Student_ID, First_name, Last_name, Exam_stage, Category, Result FROM exam WHERE Result = 'passed'";
    ...
    if ($database->numRows() > 0) {
        ...
        $insertQuery = "INSERT INTO succ_students (studentID, first_name, last_name, stage, category, result) 
                        SELECT e.Student_ID, e.First_name, e.Last_name, e.Exam_stage, e.Category, e.Result 
                        FROM exam e
                        LEFT JOIN succ_students f ON e.Student_ID = f.studentID
                        WHERE e.Result = 'passed' AND f.studentID IS NULL";
